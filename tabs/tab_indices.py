...
def render_tab_indices(df_bd_filtered, df_forma9_filtered, fecha_evaluacion, selected_activo):
    ...
    try:
        ...
        with col_left:
            st.markdown("<h5>📉 EVOLUCIÓN DE ÍNDICES</h5>", unsafe_allow_html=True)
            echarts_line = {
                "backgroundColor": "transparent",
                "tooltip": {"trigger": "axis", "axisPointer": {"type": "cross"}},
                "legend": {"data": ["Ind. Falla ON", "Ind. Falla ALS ON"], "bottom": 0, "textStyle": {"color": "#ccc"}},
                "grid": {"left": "3%", "right": "4%", "bottom": "15%", "containLabel": True},
                "xAxis": [{"type": "category", "data": months_idx, "axisLabel": {"color": "#888"}}],
                "yAxis": [{"type": "value", "axisLabel": {"formatter": "{value}%", "color": "#888"}, "splitLine": {"lineStyle": {"color": "rgba(255,255,255,0.05)"}}}],
                "series": [
            ...
            components.html(f'<div id="echarts-if-line" style="width:100%; height:350px;"></div><script src="https://cdn.jsdelivr.net/npm/echarts@5.4.3/dist/echarts.min.js"></script><script>(function(){{var myChart=echarts.init(document.getElementById("echarts-if-line"),"dark");myChart.setOption({json.dumps(echarts_line)});window.addEventListener("resize",function(){{myChart.resize();}});}})();</script>', height=370)

        with col_right:
            st.markdown("<h5>📊 OPERATIVIDAD VS FALLAS</h5>", unsafe_allow_html=True)
            p_ops = df_mensual_hist['Pozos Operativos'].tolist()
            f_tots = df_mensual_hist['Fallas Totales'].tolist()
            echarts_op = {
                "backgroundColor": "transparent",
                "tooltip": {"trigger": "axis"},
                "legend": {"data": ["Pozos Operativos", "Fallas Totales"], "bottom": 0, "textStyle": {"color": "#ccc"}},
                "grid": {"left": "3%", "right": "4%", "bottom": "15%", "containLabel": True},
                "xAxis": [{"type": "category", "data": months_idx, "axisLabel": {"color": "#888"}}],
                "yAxis": [{"type": "value", "name": "POZOS", "axisLabel": {"color": "#888"}}, {"type": "value", "name": "FALLAS", "position": "right", "splitLine": {"show": False}}],
                "series": [
                    {"name": "Pozos Operativos", "type": "line", "smooth": True, "data": p_ops, "itemStyle": {"color": "#00f2ff"}, "areaStyle": {"color": "rgba(0,242,255,0.05)"}},
                    {"name": "Fallas Totales", "type": "bar", "yAxisIndex": 1, "data": f_tots, "itemStyle": {"color": "rgba(255, 0, 255, 0.4)", "borderRadius": [4, 4, 0, 0]}}
                ]
            }
        ...
        with st.expander("📄 VER TABLA DE DATOS HISTÓRICOS Y RESUMEN", expanded=False):
            st.markdown("<div style='color:#00f2ff; font-family:Orbitron; margin-bottom:10px;'>RESUMEN DE ÍNDICES</div>", unsafe_allow_html=True)
            render_hud_table(indice_resumen_df, table_id="resumen_indices")
            
            st.markdown("<br>", unsafe_allow_html=True)
            st.markdown("<div style='color:#00f2ff; font-family:Orbitron; margin-bottom:10px;'>DETALLE MENSUAL</div>", unsafe_allow_html=True)
