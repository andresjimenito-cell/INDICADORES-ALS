...
def render_tab_performance(df_bd_filtered, df_forma9_filtered, fecha_evaluacion):
    ...
    top5['Pozo'] = top5['POZO'].astype(str)
    top5['BOPD_fmt'] = top5['BOPD'].apply(lambda x: f"{float(x):,.1f}")
    top5['RLE (d)'] = top5['RLE'].apply(lambda x: f"{float(x):.0f}d")
    top5['Efic.'] = top5['EFIC'].apply(lambda x: f"{float(x):.1f}")
    top5['Tec.'] = top5['ALS'].fillna('N/A').astype(str)
    top5['Estado'] = top5['ESTADO']
    top5_disp = top5[['Pozo', 'BOPD_fmt', 'RLE (d)', 'Efic.', 'Tec.', 'Estado']].copy()
    top5_disp = top5_disp.rename(columns={'BOPD_fmt': 'BOPD'})
    _render_custom_table(top5_disp, title="Top 5 Productores")